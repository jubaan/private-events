...
RSpec.describe User, type: :model do
  ...
  describe '#invited?' do
    ...
    it 'should return true if the user is invited to the event' do
      expect(attendee.invited?(event.id)).to be_truthy
    end
  end
  ...
  describe '#past_participations' do
    ...
    let(:hosted_events) { create_list(:event, 2, :past_events, host_id: this_user.id) }
    let(:attended_events) { create_list(:event, 2, :past_events, host_id: other_user.id) }

    (0..1).each do |i|
      let!(":appointment#{i}".to_sym) do
        create(
          :appointment,
          :status_confirmed,
          attendee_id: this_user.id,
          event_id: attended_events[i].id
        )
      end
    end
